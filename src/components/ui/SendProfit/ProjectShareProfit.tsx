...
interface ProjectShareHistoriesInterface {
  name: string;
  _id: string;
  user_name: string;
...
const ProjectShareProfit = () => {
  ...
  const [histories, setHistories] = useState<ProjectShareHistoriesInterface[]>(
    []
  );
  ...
  const fetchProjectShareHistories = async () => {
    try {
      ...
      const response = await axios.get(
        `${baseUrl}/wallet/project-share?date=${formatDate2(value)}`,
        {
          headers: {
      ...
      if (response?.data?.success) {
        setHistories(response?.data?.data?.allProjectSharePaymentByDate || []);
      }
    } catch (error) {
  ...
  const handleSendProjectShareProfit = async (userId: string) => {
    setIsLoadingForSendProfit(true);
    try {
      const response = await axios.post(
        `${baseUrl}/wallet/send-single-project-share/${userId}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleSendProjectShareProfitToAll = async () => {
    setIsLoadingForSendProfit(true);
    try {
      ...
      const response = await axios.post(
        `${baseUrl}/wallet/send-selected-project-share`,
        { projectShareProfitsIds: projectShareProfitsIds },
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      <div className="col-span-2 rounded-md py-5 bg-white">
        ...
        <div className="px-5">
          <div className="text-sm">
            Send Project Share Profit to Investors on{" "}
        ...
        <div className="flex justify-end p-3">
          <div className="flex items-center gap-3">
            <button
              ...
              onClick={handleSendProjectShareProfitToAll}
              ...
              {isLoading ? (
                ...
                histories.map((detail, i) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">{detail.project_share_amount}</td>
                    <td className="px-6 py-4">
                      {(detail.project_share_amount * 8) / 100}
                    </td>
                      ...
                      {detail.is_paid ? (
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            handleSendProjectShareProfit(detail?._id || "");
                          }}
                        >
                          {isInProgress.status &&
...
export default ProjectShareProfit;
